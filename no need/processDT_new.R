...
for(i in 1:length(pname)){
  for(r in 1:rep){
    ...
    fitmodel5 = lm(meanGZ ~ sdGenicZ, data=dt_cv5 )
    fitmodel10 = lm(meanGZ ~ sdGenicZ, data=dt_cv10 )
    
    genicEff = coef(fitmodel)[2]
    genicEff5 = coef(fitmodel5)[2]
    genicEff10 = coef(fitmodel10)[2]
    ...
    fitmodel5 = glm(genicVa ~ generation, family=Gamma(link="log"), data=out[1:6])
    fitmodel10 = glm(genicVa ~ generation, family=Gamma(link="log"), data=out[1:11])
    
    dC = 1 - exp(coef(fitmodel)[2])
    dC5 = 1 - exp(coef(fitmodel5)[2])
    dC10 = 1 - exp(coef(fitmodel10)[2])
    ...
    if(i==1 & r==1){
      output<- as.data.table(cbind(proname,nGeneration,BVtype,ac,relM,TD,ggst,ggstg,genicEff,geneticEff,genicEff5,genicEff10,gg0,gg5,gg10,gg15,accallcand,accfam,variance,genicVariance,genicNe,genicNe5,genicNe10,geneticNe))
    }else{
      output = rbind(output,as.data.table(cbind(proname,nGeneration,BVtype,ac,relM,TD,ggst,ggstg,genicEff,geneticEff,genicEff5,genicEff10,gg0,gg5,gg10,gg15,accallcand,accfam,variance,genicVariance,genicNe,genicNe5,genicNe10,geneticNe)))
    }
  }
}
colnames(output) = c("pname","nGeneration","BV","tac","relMat","TargetD","ggst","ggstg","genicEff","geneticEff","genicEff5","genicEff10","gg0","gg5","gg10","gg15","acforallcand","acforfam","Va","genicVa","genicNe","genicNe5","genicNe10","geneticNe")
...
result_mean <- output[, lapply(.SD, mean), by = .(pname,nGeneration,BV,tac,relMat,TargetD)]

result_sd <- output[, lapply(.SD, sd), by = .(pname,nGeneration,BV,tac,relMat,TargetD)]

bool = (!colnames(result_sd)%in%c("pname","nGeneration","BV","tac","relMat","TargetD"))

colnames(result_sd)[bool] = paste(colnames(result_sd)[bool],"se",sep = "")

zt1 <- merge(result_mean, result_sd, by = c("pname","nGeneration","BV","tac","relMat","TargetD"))
...
#this should be the addtional figure for my own revised version
#Additional file 6, Figrue 18
pv = "genicNe10"
pvse = "genicNe10se"
nG = 10
zt = rbind(zt1[nGeneration==nG&relMat=="G"&tac!=0&BV == "old",],zt1[nGeneration==nG&relMat=="A"&tac==0&BV == "old",],zt1[nGeneration==nG&relMat=="N"&BV == "old",])
# zt$tac <- c(0, 1, 0.1, 0.3, 0.5, 0.7, 0.9)[match(zt$tac, c(0, 1, 2, 3, 4, 5, 6))]
zt$TargetD = as.character(zt$TargetD)
zt$value = zt[,..pv]
zt$valuese = zt[,..pvse]
zt$tac = as.character(zt$tac)
P<- ggplot(data = zt,aes(x=tac,y=value,group = TargetD, color = TargetD))+
  geom_point()+
  geom_line()+
  #scale_shape_manual(values = c(0,3,15,1),na.translate=FALSE)+
  xlab("The accuracy within family")+
...
P <- ggplot(data = zt, aes(x = tac, y = value, group = TargetD, fill = TargetD)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +  # 确保条形图分组不重叠
...
P
ggsave("FigrueS18.pdf", P , width = 15, height = 5, dpi = 300)

zt[tac == "0",]
P1 <- ggplot(data = zt, aes(x = tac, y = value, group = TargetD, fill = TargetD)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +  # 确保条形图分组不重叠
  xlab("Within-family accuracy") +
  ylab("Genetic gain") +
  theme_zg() +
...
# ggsave("ConEff10.pdf", P , width = 15, height = 5, dpi = 300)
# ggsave("Gain10.pdf", P , width = 15, height = 5, dpi = 300)
# ggsave("Va10.pdf", P , width = 15, height = 5, dpi = 300)
# ggsave("Ne10.pdf", P , width = 15, height = 5, dpi = 300)
P<- ggplot(data = zt[tac == 0,],aes(x=TargetD,y=value,group = tac))+
  geom_point()+
  geom_line()+
  #scale_shape_manual(values = c(0,3,15,1),na.translate=FALSE)+
  xlab("The accuracy within family")+
...
t.test(x = output[nGeneration==20&relMat=="A"&tac==0&BV=="old"&TargetD==45,genicEff],
       y = output[nGeneration==20&relMat=="A"&tac==0&BV=="old"&TargetD==65,genicEff])

t.test(x = output[nGeneration==20&relMat=="A"&tac==0&TargetD==45,genicEff],
y = output[nGeneration==20&BV=="old"&relMat == "G"&TargetD==45&tac==0.5,genicEff])

t.test(x = output[nGeneration==20&BV=="old"&relMat == "G"&TargetD==45&tac==0.7,genicEff],
       y = output[nGeneration==20&BV=="old"&relMat == "G"&TargetD==45&tac==1,genicEff])
